...
export async function getGithubDatabase(): Promise<Optional<[Blob, string]>> {
	...
	if (content.isOk()) {
		const contentJson = await content.ok().json();
		const downloaded = await getBlob(contentJson[0].git_url);
		if (downloaded.isOk()) {
			const downloadedJson = await downloaded.ok().json();
			const blob = await base64ToBlob(downloadedJson.content);
			return Optional.of([blob, downloadedJson.sha]);
		}
...
async function handleRepositoryDownloadFailure(
	error: FetchError,
	config: Configuration,
): Promise<Optional<[Blob, string]>> {
	if (error.status == 404) {
